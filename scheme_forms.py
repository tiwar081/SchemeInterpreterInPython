...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        temp = Pair(formals, body)
        l_procedure = do_lambda_form(temp, env)
        env.bindings[name] = l_procedure
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if is_scheme_false(x) or expressions.rest is nil:
        return x
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions.rest is nil:
        ...
        x = scheme_eval(expressions.first, env)
    if is_scheme_true(x) or expressions.rest is nil:
        return x
    else:
...
def make_let_frame(bindings, env):
    ...
    while temp is not nil:
        ...
        names = Pair(temp.first.first, names)
        vals = Pair(scheme_eval(temp.first.rest.first, env), vals)
        temp = temp.rest
